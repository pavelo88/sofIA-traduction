    ...
    <Dialog>
      ...
      <DialogContent className="glass-panel border-white/10 bg-zinc-950/95 backdrop-blur-3xl text-white rounded-[2.5rem] max-w-lg p-0 overflow-hidden">
        ...
        <div className="p-8 space-y-8 max-h-[75vh] overflow-y-auto custom-scrollbar">
          ...
          {/* 2. CONFIGURACIÓN DE RUTA (IDIOMAS) */}
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-[9px] uppercase tracking-widest text-white/50 ml-1">Idioma que hablo (Nativo)</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[9px] uppercase tracking-widest text-white/50 ml-1">Idioma que deseo aprender</Label>
          ...
          {/* 3. SELECTOR DE INFRAESTRUCTURA DE IA */}
          <div className="space-y-4">
            <h3 className="text-[10px] uppercase tracking-[0.3em] font-bold text-white/30 flex items-center gap-2">
            ...
            <div className="grid gap-4">
              ...
              <button 
                ...
                className={cn(
                  "flex flex-col gap-4 p-5 rounded-[2rem] border transition-all duration-300 text-left relative overflow-hidden group",
                  aiEngineMode === 'cloud' 
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={cn("p-2 rounded-xl transition-colors", aiEngineMode === 'cloud' ? "bg-primary text-white" : "bg-white/10 text-white/40")}>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-[9px] text-white/50 leading-relaxed italic">
                    Consumo: 1 crédito por mensaje. Máxima precisión conversacional.
                  </p>
                </div>
                ...
                <Button 
                  ...
                  className="w-full mt-2 h-10 rounded-xl bg-green-500/10 hover:bg-green-500/20 text-green-400 border border-green-500/20 gap-2 text-[10px] font-headline uppercase tracking-widest"
                >
                  {isWatchingAd ? (
                    <Zap className="w-3 h-3 animate-spin" />
                  ) : (
                    <>
                      <PlayCircle className="w-4 h-4" /> Ver Anuncio (+5 Créditos)
                    </>
                  )}
                </Button>
              </button>
              ...
              {/* OPCIÓN LOCAL */}
              <button 
                onClick={() => { setAiEngineMode('local'); handleUpdate('aiEngineMode', 'local'); }}
                className={cn(
                  "flex flex-col gap-3 p-5 rounded-[2rem] border transition-all duration-300 text-left relative group",
                  aiEngineMode === 'local' 
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={cn("p-2 rounded-xl transition-colors", aiEngineMode === 'local' ? "bg-secondary text-white" : "bg-white/10 text-white/40")}>
                  ...
                  <Badge className="bg-secondary text-secondary-foreground text-[8px] font-black tracking-widest uppercase">¡Gratis!</Badge>
                </div>
                <div className="flex items-center gap-2 text-[9px] text-white/50 leading-relaxed">
        ...
        <div className="p-8 pt-4 border-t border-white/5 bg-white/[0.02]">
          <Button className="w-full h-14 rounded-2xl bg-primary hover:bg-primary/80 font-headline font-bold text-white shadow-lg shadow-primary/20">
            Confirmar Cambios Espaciales
          </Button>
        </div>
      </DialogContent>
    </Dialog>
